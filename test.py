...
phrase_text = phrase.split()

input = create_input(phrase_text, word_to_idx)

out = model(input)

results = torch.argmax(out, dim=1)

for i in range(len(phrase_text)):
    print("{} [{}]".format(idx_to_tag[results[i].item()], "Unknown" if input[i] == len(word_to_idx) else "Known"))
